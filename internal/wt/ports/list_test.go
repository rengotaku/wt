...
func TestLiveCell(t *testing.T) {
	tests := []struct {
		...
		{
			// A headless worker is Running but not Listening: it must appear
			// (not "idle") and be marked with "*" since it binds no port.
			name: "headless running worker",
			states: []PortState{
				{Port: 9160, Listening: true, Proc: "python", PID: 123},
				{Port: 9161, Running: true, Proc: "worker", PID: 456},
			},
